...
```sql
select student.s_id,student.s_name,AVG(score.s_score) as avg_score from
  Student student
    join Score score on student.s_id = score.s_id
GROUP BY student.s_id,student.s_name HAVING AVG(score.s_score)>=60;
...
```sql
select student.s_id,student.s_name, count(score.c_id) as sum_course,SUM(score.s_score) as sum_score
from Student student
       left join Score score on student.s_id = score.s_id
group by student.s_id, student.s_name;
```
...
```sql
select count(teacher.t_id) from Teacher teacher where teacher.t_name like '李%';
```
...
select student.*
from Student student
       join Score score on score.s_id = student.s_id
       join Course course on course.c_id = score.c_id
       join Teacher teacher on teacher.t_id = course.t_id
where teacher.t_name = '张三';

select student.* from
  Student student
    join Score score on student.s_id=score.s_id where score.c_id in(
...
select * from
  Student student
where student.s_id not in (
  select student.s_id from Student student join Score score on student.s_id= score.s_id where score.c_id in (
    select c_id from Course where t_id =(
      select t_id from Teacher where t_name = '张三')));
```
...
from
  ...
  Score score01,
  Score score02
where student.s_id = score01.s_id
  and student.s_id = score02.s_id
  and score01.c_id = '01'
  and score02.c_id = '02';
```
...
where student.s_id in (
  select score01.s_id from Score score01 where score01.c_id='01'
)
and student.s_id not in(
  select score02.s_id from Score score02 where score02.c_id='02'
);
```
### 11、查询没有学全所有课程的同学的信息 
...
```text
select student.* from Student student where student.s_id in(
      select distinct score_stu.s_id from Score score_stu where score_stu.c_id in(
        select score.c_id from Score score where score.s_id='01'
      )
);
```
